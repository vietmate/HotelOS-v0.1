...
export const EmployeesView: React.FC<EmployeesViewProps> = ({ 
  ...
  const handleAddEmployeeLocal = () => {
    ...
    const emp: Employee = {
        ...
        name: newEmp.name,
        role: (newEmp.role as EmployeeRole) || 'Reception',
        hourlyRate: 0,
        isWorking: false,
        phone: '',
        reviewsHistory: {} 
  ...
  const handleDeleteEmployeeLocal = (id: string) => {
      if (!confirm(t.employees.deleteStaffConfirm)) return;
      onDeleteEmployee(id);
  };

  const handleIncrementReview = (empId: string) => {
  ...
  const handleResetLeaderboardMonth = () => {
  ...
  const handleDeleteEntryLocal = (id: string) => {
             ...
             {isAddingEmp && (
                 <div className="p-4 border-b border-slate-100 dark:border-slate-700 bg-indigo-50/50 dark:bg-indigo-900/10 animate-in slide-in-from-top-2">
                     <div className="space-y-3">
                         ...
                         <div className="flex gap-2 justify-end"><button onClick={() => setIsAddingEmp(false)} className="text-xs font-bold text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200">{t.employees.cancel}</button><button onClick={handleAddEmployeeLocal} className="text-xs font-bold bg-indigo-600 text-white px-3 py-1.5 rounded hover:bg-indigo-700 transition-colors">{t.employees.save}</button></div>
                     </div>
                 </div>
             )}
