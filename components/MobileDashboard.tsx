...
export const MobileDashboard: React.FC<MobileDashboardProps> = ({ rooms, onRoomClick, lang }) => {
  ...
  // Filter Arrivals: Look into the array of future reservations
  const arrivals = rooms.filter(r => 
    r.futureReservations && r.futureReservations.some(res => res.checkInDate === todayStr)
  );
    ...
    <div className="p-4 space-y-6 pb-24">
      ...
      <div className="space-y-3">
          ...
          {departures.length === 0 ? (
              ...
              <div className="grid gap-3">
                  {departures.map(room => (
                      <div key={room.id} onClick={() => onRoomClick(room)} className="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm border-l-4 border-rose-500 flex justify-between items-center cursor-pointer active:scale-95 transition-transform">
                          <div>
                          ...
                          <button className="bg-rose-50 dark:bg-rose-900/20 text-rose-600 p-2 rounded-full"><ArrowRight className="w-5 h-5" /></button>
                      </div>
                  ))}
              </div>
          )}
      ...
      <div className="space-y-3">
          ...
          {arrivals.length === 0 ? (
              ...
              <div className="grid gap-3">
                  {arrivals.map(room => {
                      const todayRes = room.futureReservations?.find(res => res.checkInDate === todayStr);
                      return (
                      <div key={room.id} onClick={() => onRoomClick(room)} className="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm border-l-4 border-emerald-500 flex justify-between items-center cursor-pointer active:scale-95 transition-transform">
                          <div>
                              <div className="flex items-center gap-2 mb-1">
                                  <span className="font-black text-lg text-slate-800 dark:text-white">{room.number}</span>
                                  <span className="text-xs text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 px-2 py-0.5 rounded-full font-bold">Arrival</span>
                              </div>
                              <div className="text-sm font-bold text-slate-700 dark:text-slate-300">{todayRes?.guestName}</div>
                          </div>
                          <button className="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 p-2 rounded-full"><ArrowRight className="w-5 h-5" /></button>
                      </div>
                  )})}
              </div>
          )}
